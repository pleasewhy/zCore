...
pub fn run(args: Vec<String>, envs: Vec<String>, rootfs: Arc<dyn FileSystem>) -> Arc<Process> {
    ...
    let job = Job::root();
    let proc = block_on(Process::create_linux(&job, rootfs.clone())).unwrap();
    ...
    // 调用 zircon-object/src/task/thread.start 设置好要执行的 thread
    let (entry, sp) = block_on(loader.load(&proc.vmar(), &data, args, envs, path)).unwrap();
